...
function [time_table_0, solution_tape] = Get_Table_Simulation_iiwa(varargin)

Parser = inputParser;
Parser.FunctionName = 'Get_Table_Simulation';
...
Q = 1000*eye(2 * n);
R = 0.1*eye(Handler_dynamics_generalized_coordinates_model.dof_control);
...
if Parser.Results.Enable_tester  
    [~,~,~] = Control_Tester_iiwa(time_table_0,x_table, K_table, solution_tape,Q,R);
else 
